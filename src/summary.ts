import {groupByModel, groupByPeriod, groupBySource, totalsFor} from './aggregate.js';
...
const table = (groups: UsageGroup[], name: 'model' | 'period' | 'source'): string => {
  const labelWidth = name === 'model' ? 34 : 14;
  const heading = [
  ...
  const rows = groups.map(group => {
    const label = name === 'model' ? `${group.source}/${group.model}` : group.key;
    return [
...
export const formatSummary = (result: ScanResult, period: Period): string => {
  ...
  return [
    `llmusage · ${result.files.length} files · ${totals.sessions} sessions · ${formatTokens(totals.totalTokens)} tokens · ${formatUsd(totals.cost)}${estimate}`,
    '',
